...
console.log('2. Compiling reason-standard');
// Bundle BuckleScript compiler with reason-standard .cmi and .cmj files:
// 1. compile reason-standard with bsb
// 2. serialize reason-standard .cmi and .cmj files
...
childProcess.execSync('../bs-platform/lib/bsb.exe -clean-world -make-world', {
  cwd: path.join(__dirname, 'node_modules', 'reason-standard'),
  encoding: 'utf8',
  stdio: [0, 1, 2],
  shell: true,
});

const ReasonStandardBsDir = path.join(
  __dirname,
  'node_modules',
  'reason-standard',
  'lib',
...
const moduleNames = ['Standard'];
const modules = moduleNames.map(name => ({
  cmi: path.join(ReasonStandardBsDir, name + '.cmi'),
  cmj: path.join(ReasonStandardBsDir, name + '.cmj'),
}));

const bsFilename = path.join(playgroundDir, 'bs', 'exports.js');
const bsReasonStandardFilename = path.join(assetsJsDir, 'bsReasonStandard.js');
fs.copyFileSync(bsFilename, bsReasonStandardFilename);
modules.forEach(module => {
  const cmi = module.cmi;
  const cmiBasename = path.basename(cmi);
  fs.appendFileSync(
    bsReasonStandardFilename,
    `ocaml.load_module("/static/cmis/${cmiBasename}", `,
  );
  serializeBinary(cmi, bsReasonStandardFilename);
  const cmj = module.cmj;
  const cmjBasename = path.basename(cmj);
  fs.appendFileSync(bsReasonStandardFilename, `, "${cmjBasename}", `);
  serializeBinary(cmj, bsReasonStandardFilename);
  fs.appendFileSync(bsReasonStandardFilename, ');\n');
});

console.log('3. Bundling reason-standard');

const standardBundle = browserify();
const reasonStandardJsDir = path.join(
  __dirname,
  'node_modules',
  'reason-standard',
  'src',
);
let reasonStandardDirFiles;
try {
  reasonStandardDirFiles = fs.readdirSync(reasonStandardJsDir);
} catch (e) {
  console.log(`
** There's an error while reading ${reasonStandardJsDir}.`);
  process.exit(1);
}
reasonStandardDirFiles
  .filter(file => path.extname(file) === '.js')
  .forEach(file => {
    const exposedRequireName =
      './stdlib/' + lowercaseFirstLetter(path.basename(file));
    standardBundle.require(path.join(reasonStandardJsDir, file), {
      expose: exposedRequireName,
    });
  });
...
standardBundle
  ...
  .bundle()
  .pipe(
    fs.createWriteStream(path.join(assetsJsDir, 'reasonStandardBundle.js')),
  );
