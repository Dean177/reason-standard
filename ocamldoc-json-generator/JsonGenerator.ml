  ...
  method list_types = list_types
    ...
    method create_fully_qualified_idents_links m_name s : string =
      ...
      let f (str_t: string) =
        ...
        let s_final = Odoc_info.apply_if_equal
            ...
            match_s
            rel
        in if known_type || (retry && StringSet.mem match_s known_types_names) then
          "<a href=\""^(Naming.complete_target Naming.mark_type match_s)^"\">"^
           s_final^
           "</a>"
        else if known_class || (retry && StringSet.mem match_s known_classes_names) then
    ...
    method create_fully_qualified_module_idents_links m_name s : string =
      let f str_t =
        ...
        in
        if known_module ||
           (retry && StringSet.mem match_s known_modules_names) then
          let (html_file, _) = Naming.html_files match_s in
          "<a href=\""^html_file^"\">"^s_final^"</a>"
        else
          s_final
    ...
    method json_of_modtype ?(info=true) ?(complete=true) ?(with_link=true) (mt : t_module_type) =
      ...
      (tagged "ModuleType" (obj [
        ...
        ("signature", nullable (fun (m_type : Types.module_type) : Json.t -> string (Odoc_info.string_of_module_type ~complete:true m_type))  mt.mt_type);
        ("elements", array
    ...
    method generate (module_list: t_module list) =
      ...
      let module_types : t_module_type list = Odoc_info.Search.module_types module_list in
      known_modules_names <-
        List.fold_left
      ...
      |> function 
         ...
         | entrypoint :: linkedModules ->
            ...
            toBuffer buffer (obj [
              ...
              ("modules", obj (List.map (fun modu -> 
                (modu.m_name, self#json_of_module modu)) linkedModules));
