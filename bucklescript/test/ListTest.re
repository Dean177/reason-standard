...
let suite = suite("List", () => {
  ...
  describe("drop", () => {
    ...
    test("zero elements", () =>
      expect(drop([1, 2, 3], ~count=0)) |> toEqual(Eq.(list(int)), [1, 2, 3])
    );

    test("the first element", () =>
      expect(drop([1, 2, 3], ~count=1)) |> toEqual(Eq.(list(int)), [2, 3])
    );
  ...
  describe("findIndex", () => {
    ...
    test("returns `None` if `f` returns false for all elements ", () => {
      expect(findIndex(~f=(_, _) => false, [0, 2, 4, 8])) |> toEqual(Eq.(option(pair(int, int))), None)
    });
  ...
  describe("map2", () => {
    ...
    test("map2 two elements", () => {
      expect(map2(~f=(+), [1, 2], [1, 2])) |> toEqual(Eq.(list(int)), [2, 4])
    });
  });

  describe("mapI", () => {
    test("mapI empty list", () => {
      expect(mapI(~f=(i, _) => i, [])) |> toEqual(Eq.(list(int)), [])
    });
    test("mapI one element", () => {
      expect(mapI(~f=(i, _) => i, ['a'])) |> toEqual(Eq.(list(int)), [0])
    });
    test("mapI two elements", () => {
      expect(mapI(~f=(i, _) => i, ['a', 'b'])) |> toEqual(Eq.(list(int)), [0, 1])
    });
  });
  ...
  describe("sliding", () => {
    ...
    test("size 2", () => {
      ...
      |> toEqual(Eq.(list(list(int))), [[1, 2], [2, 3], [3, 4], [4, 5]])
    });

    test("step 3 ", () => {
      expect(sliding([1, 2, 3, 4, 5], ~size=3))
      |> toEqual(Eq.(list(list(int))), [[1, 2, 3], [2, 3, 4], [3, 4, 5]])
    });
    ...
    test("step 7", () => {
      expect(sliding([1, 2, 3, 4, 5], ~size=7)) |> toEqual(Eq.(list(list(int))), [])
    });
  });
  ...
  describe("intersperse", () => {
    test("intersperse empty list", () => {
      expect(intersperse([], ~sep="on")) |> toEqual(Eq.(list(string)), [])
    });
    test("intersperse one turtle", () => {
      expect(intersperse(~sep="on", ["turtles"])) |> toEqual(Eq.(list(string)), ["turtles"])
    });
    test("intersperse three turtles", () => {
      expect(intersperse(~sep="on", ["turtles", "turtles", "turtles"]))
      |> toEqual(Eq.(list(string)), ["turtles", "on", "turtles", "on", "turtles"])
    });
  });
  ...
  describe("initial", () => {
    ...
    test("two elements", () => {
      expect(initial(['a', 'b'])) |> toEqual(Eq.(option(list(char))), Some(['a']))
    });
  });
  ...
  describe("append", () => {
    ...
    test("append empty list", () => {
      expect(append([], ["turtles"])) |> toEqual(Eq.(list(string)), ["turtles"])
    });
    test("append empty list", () => {
      expect(append(["turtles"], [])) |> toEqual(Eq.(list(string)), ["turtles"])
    });
    test("append two lists", () => {
      expect(append(["on"], ["turtles"])) |> toEqual(Eq.(list(string)), ["on", "turtles"])
    });
  });

  describe("folds", () => {
    test("empty list", () => {
      expect(fold(~f=cons, ~initial=[], [])) |> toEqual(Eq.(list(int)), [])
    });
    test("one element", () => {
      expect(fold(~f=cons, ~initial=[], [1])) |> toEqual(Eq.(list(int)), [1])
    });
    test("three elements", () => {
      expect(fold(~f=cons, ~initial=[], [1, 2, 3])) |> toEqual(Eq.(list(int)), [3, 2, 1])
    });
    test("foldr empty list", () => {
      expect(foldRight(~f=cons, ~initial=[], [])) |> toEqual(Eq.(list(int)), [])
    });
    test("foldr one element", () => {
      expect(foldRight(~f=cons, ~initial=[], [1])) |> toEqual(Eq.(list(int)), [1])
    });
  ...
  describe("updateAt", () => {
    test("updateAt index smaller 0", () => {
      expect(updateAt(~index=-1, ~f=x => x + 1, [1, 3])) |> toEqual(Eq.(list(int)), [1, 3])
    });
    test("updateAt empty list", () => {
      expect(updateAt(~index=0, ~f=x => x + 1, [])) |> toEqual(Eq.(list(int)), [])
    });
    test("updateAt empty list", () => {
      expect(updateAt(~index=2, ~f=x => x + 1, [])) |> toEqual(Eq.(list(int)), [])
    });
    test("updateAt inside the list", () => {
      expect(updateAt(~index=1, ~f=x => x + 1, [1, 3])) |> toEqual(Eq.(list(int)), [1, 4])
    });
    test("updateAt in the front", () => {
      expect(updateAt(~index=0, ~f=x => x + 1, [1, 3])) |> toEqual(Eq.(list(int)), [2, 3])
    });
    test("updateAt after end of list", () => {
      expect(updateAt(~index=4, ~f=x => x + 1, [1, 3])) |> toEqual(Eq.(list(int)), [1, 3])
    });
  });
  ...
  describe("concatenate", () => {
    ...
    test("several lists", () => {
      expect(concatenate([[1], [2], [3]])) |> toEqual(Eq.(list(int)), [1, 2, 3])
    });
    test("several lists", () => {
      expect(concatenate([[1], [], [2], [], [3]])) |> toEqual(Eq.(list(int)), [1, 2, 3])
    });
  ...
  describe("removeAt", () => {
    test("removeAt index smaller 0", () => {
      expect(removeAt(~index=-1, [1, 3])) |> toEqual(Eq.(list(int)), [1, 3])
    });
    ...
    test("removeAt index 1", () => {
      expect(removeAt(~index=1, [1, 3])) |> toEqual(Eq.(list(int)), [1])
    });
    test("removeAt index 0", () => {
      expect(removeAt(~index=0, [1, 3])) |> toEqual(Eq.(list(int)), [3])
    });
    test("removeAt after end of list", () => {
      expect(removeAt(~index=4, [1, 3])) |> toEqual(Eq.(list(int)), [1, 3])
    });
  });
});
