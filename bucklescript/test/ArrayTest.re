...
let suite = suite("Array", () => {
  ...
  describe("initialize", () => {
    test("create empty array", () => {
      expect(initialize(0, ~f=Fun.identity)) |> toEqual(Eq.(array(int)), [||])
    });

    test("negative length gives an empty array", () => {
      expect(initialize(-1, ~f=Fun.identity)) |> toEqual(Eq.(array(int)),[||])
    });

    test("create array initialize", () => {
      expect(initialize(3, ~f=Fun.identity)) |> toEqual(Eq.(array(int)),[|0, 1, 2|])
    });
  });
  ...
  describe("repeat", () => {
    ...
    test("create array of ints", () => {
      expect(repeat(0, ~length=3)) |> toEqual(Eq.(array(int)),[|0, 0, 0|])
    });
  ...
  describe("range", () => {
    ...
    test("can start from negative values", () => {
      expect(range(~from=-2, 3)) |> toEqual(Eq.(array(int)), [|(-2), (-1), 0, 1, 2|])
    });
  ...
  describe("ofList", () => {
    test("transforms a list into an array of the same elements", () => {
      expect((ofList([1, 2, 3]))) |> toEqual(Eq.(array(int)), [|1, 2, 3|])
    })
  });
  ...
  describe("toIndexedList", () => {
    test("returns an empty list for an empty array", () => {
      expect(toIndexedList([||])) |> toEqual(Eq.(list(pair(int, int))), [])
    });
  ...
  describe("getAt", () => {
    ...
    test("returns None for an out of bounds index", () => {
      expect(getAt(~index=5, [|0, 1, 2|])) |> toEqual(Eq.(option(int)), None)
    });
  ...
  describe("mapI", () => {
    test("equals an array literal of the same value", () => {
      expect(mapI(~f=( * ), [|5, 5, 5|])) |> toEqual(Eq.(array(int)), [|0, 5, 10|])
    })
  });
  ...
  describe("sliding", () => {
    ...
    test("step 7", () => {
      expect(sliding([|1, 2, 3, 4, 5|], ~size=7)) |> toEqual(Eq.(array(array(int))), [||])
    });
  });
  ...
  describe("find", () => {
    ...
    test("returns `None` if `f` returns false for all elements ", () => {
      expect(find(~f=Int.isOdd, [|0, 2, 4, 8|])) |> toEqual(Eq.(option(int)), None)
    });

    test("returns `None` for an empty array", () => {
      expect(find(~f=Int.isEven, [||])) |> toEqual(Eq.(option(int)), None)
    });
  });
  ...
  describe("slice", () => {
    ...
    test("a positive `from`", () => {
      expect(slice(~from=1, numbers)) |> toEqual(Eq.(array(int)), [|1, 2, 3, 4|])
    });

    test("a negative `from`", () => {
      expect(slice(~from=-1, numbers)) |> toEqual(Eq.(array(int)), [|4|])
    });
    ...
    test("a positive `to_`", () => {
      expect(slice(~from=0, ~to_=3, numbers)) |> toEqual(Eq.(array(int)), [|0, 1, 2|])
    });

    test("a negative `to_`", () => {
      expect(slice(~from=1, ~to_=-1, numbers)) |> toEqual(Eq.(array(int)), [|1, 2, 3|])
    });

    testAll("`to_` >= length", positiveArrayLengths, to_ =>
      expect(slice(~from=0, ~to_, numbers)) |> toEqual(Eq.(array(int)), numbers)
    );

    testAll("`to_` <= negative `length`", negativeArrayLengths, to_ =>
      expect(slice(~from=0, ~to_, numbers)) |> toEqual(Eq.(array(int)), [||])
    );

    test("both `from` and `to_` are negative and `from` < `to_`", () =>
      expect(slice(~from=-2, ~to_=-1, numbers)) |> toEqual(Eq.(array(int)), [|3|])
    );

    test("works `from` >= `to_`", () => {
      expect(slice(~from=4, ~to_=3, numbers)) |> toEqual(Eq.(array(int)), [||])
    });
  });
  ...
  describe("fold", () => {
    ...
    test("works the order of arguments to `f` is important", () => {
      expect(fold(~f=List.cons, ~initial=[], [|1, 2, 3|]))
  ...
  describe("foldRight", () => {
    ...
    test("foldRight", () => {
      expect(
        foldRight(~f=(+), ~initial=0, repeat(~length=3, 5)),
      )
      |> toEqual(Eq.int, 15)
    });
    ...
    test("works the order of arguments to `f` is important", () => {
      expect(foldRight(~f=List.cons, ~initial=[], [|1, 2, 3|]))
