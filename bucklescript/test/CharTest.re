  ...
  suite("Char", () => {
    ...
    describe("ofCode", () => {
      test("valid ASCII codes return the corresponding character", () => {
        expect(ofCode(97)) |> toEqual(Eq.(option(char)), Some('a'))
      });
      test("negative integers return None", () => {
        expect(ofCode(-1)) |> toEqual(Eq.(option(char)), None)
      });
      test("integers greater than 255 return None", () => {
        expect(ofCode(256)) |> toEqual(Eq.(option(char)), None)
      });
    });
    ...
    describe("ofString", () => {
      test("one-length string return Some", () => {
        expect(ofString("a")) |> toEqual(Eq.(option(char)), Some('a'))
      });
      test("multi character strings return None", () => {
        expect(ofString("abc")) |> toEqual(Eq.(option(char)), None)
      });
      test("zero length strings return None", () => {
        expect(ofString("")) |> toEqual(Eq.(option(char)), None)
      });
    });
    ...
    describe("isPrintable", () => {
      ...
      test("returns false for non-printable character", () => {
        expect(ofCode(31) |> Option.map(~f=isPrintable))
