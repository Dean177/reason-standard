...
let suite =
  suite("Int", () => {    
    ...
    describe("divide", () => {
      ...
      test("-x /. 0", () => {
        expect(Int.((-8) /. 0) == Float.negativeInfinity) |> toEqual(Eq.bool, true)
      });
    });
    ...
    describe("modulo", () => {
      test("documentation examples", () => {
        expect(Array.map([|-4, -3, -2, -1, 0, 1, 2, 3, 4|], ~f=Int.modulo(~by=3)))
    ...
    describe("ofString", () => {
      test("0", () => {
        expect(Int.ofString("0")) |> toEqual(Eq.(option(int)), Some(0))
      });
      test("-0", () => {
        expect(Int.ofString("-0")) |> toEqual(Eq.(option(int)), Some(-0))
      });
      test("42", () => {
        expect(Int.ofString("42")) |> toEqual(Eq.(option(int)), Some(42))
      });
      test("123_456", () => {
        expect(Int.ofString("123_456"))
      ...
      test("0XFF", () => {
        expect(Int.ofString("0XFF")) |> toEqual(Eq.(option(int)), Some(255))
      });
      test("0X000A", () => {
        expect(Int.ofString("0X000A")) |> toEqual(Eq.(option(int)), Some(10))
      });
      test("Infinity", () => {
        expect(Int.ofString("Infinity")) |> toEqual(Eq.(option(int)), None)
      });
      test("-Infinity", () => {
        expect(Int.ofString("-Infinity")) |> toEqual(Eq.(option(int)), None)
      });
      test("NaN", () => {
        expect(Int.ofString("NaN")) |> toEqual(Eq.(option(int)), None)
      });
      test("abc", () => {
        expect(Int.ofString("abc")) |> toEqual(Eq.(option(int)), None)
      });
      test("--4", () => {
        expect(Int.ofString("--4")) |> toEqual(Eq.(option(int)), None)
      });
      test("empty string", () => {
        expect(Int.ofString(" ")) |> toEqual(Eq.(option(int)), None)
      });
    });
