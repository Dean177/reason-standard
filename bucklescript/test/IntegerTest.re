...
let suite = suite("Integer", () => {
  ...
  describe("add", () => {
    testAll("add", [add, (+)], op => {
      expect(op(ofInt(3002), ofInt(4004))) |> toEqual(Eq.integer, ofInt(7006))
    })
  });
  ...
  describe("ofString", () => {
    ...
    test("0XFF", () => {
      expect(ofString("0XFF")) |> toEqual(Eq.(option(integer)), Some(ofInt(255)))
    });
    test("0X000A", () => {
      expect(ofString("0X000A")) |> toEqual(Eq.(option(integer)), Some(ofInt(10)))
    });
    test("Infinity", () => {
