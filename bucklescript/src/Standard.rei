...
module Result: {
  ...
  */
  let join: t(t('a, 'error), 'error) => t('a, 'error);
  ...
  */
  let equal: (('error, 'error) => bool, ('ok, 'ok) => bool, t('ok, 'error), t('ok, 'error)) => bool;
  ...
  */
  let compare: (('error, 'error) => int, ('ok, 'ok) => int, t('ok, 'error), t('ok, 'error)) => int;
