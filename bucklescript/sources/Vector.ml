      ...
      (let pos = (index lsr shift) land bitMask in
       ...
       then
         ...
         (let value = IArray.get tree pos in
          ...
          IArray.set tree pos ((SubTree (newSub))[@explicit_arity ])) : 
      'a t)
...
let rec compressNodes (nodes : 'a Tree.node list) acc =
  ...
  let newAcc = ((Tree.SubTree (node))[@explicit_arity ]) :: acc in
...
let get (t : 'a t) (index : int) =
  (if (index < 0) || (index >= t.length)
   then None
     ...
     then ((Some ((IArray.get t.tail index)))[@explicit_arity ])
     else ((Some ((Tree.get t.tree t.startShift index)))[@explicit_arity ]) : 
  'a option)
   ...
   then
     ...
     (if hasOverflown
      then
        ...
        let newTree =
          Tree.insertTail
            (IArray.singleton ((Tree.SubTree ((t.tree)))[@explicit_arity ]))
