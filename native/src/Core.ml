...
module Char = struct
    ...
    let ofCode (i : int) =
      (if (0 <= i) && (i <= 255)
       then ((Some ((Char.chr i))))
       else None : char option)
...
module Fun =
  struct
    ...
    let rec times n ~f  = if n <= 0 then () else (f (); times (n - 1) ~f)
    let negate f a = not (f a)
    let forever f = while true do f () done
  end
module Container =
  struct
    module type Sum  = sig type t val zero : t val add : t -> t -> t end
  end
module Tuple =
  struct
    type ('a, 'b) t = ('a * 'b)
...
module Tuple3 =
  struct
    ...
    let rotateLeft ((a, b, c) : ('a * 'b * 'c)) =
      ((b, c, a) : ('b * 'c * 'a))
    let rotateRight ((a, b, c) : ('a * 'b * 'c)) =
      ((c, a, b) : ('c * 'a * 'b))
    let curry (f : ('a * 'b * 'c) -> 'd) (a : 'a) (b : 'b) (c : 'c) =
      (f (a, b, c) : 'd)
    let uncurry (f : 'a -> 'b -> 'c -> 'd) ((a, b, c) : ('a * 'b * 'c)) =
      (f a b c : 'd)
    ...
    let compare compareFirst compareSecond compareThird (a, b, c)
      (a', b', c') =
      match compareFirst a a' with
      | 0 ->
...
module List =
  struct
    ...
    let sum (type a) (a : a t)
      ((module M)  : (module Container.Sum with type t = a)) =
    ...
    let takeWhile (l : 'a list) ~f:(f : 'a -> bool)  =
      (let rec takeWhileHelper acc l' =
         ...
         | x::rest ->
             ...
             else Base.List.rev acc in
       takeWhileHelper [] l : 'a list)
    let drop t ~count  = Base.List.drop t count
    let rec dropWhile (l : 'a list) ~f:(f : 'a -> bool)  =
      (match l with
       | [] -> []
       | x::rest -> if f x then dropWhile ~f rest else l : 'a list)
    ...
    let rec last (l : 'a list) =
      (match l with
       | [] -> None
       | a::[] -> ((Some (a)))
       | _::tail -> last tail : 'a option)
    let append (l1 : 'a list) (l2 : 'a list) =
      (Base.List.append l1 l2 : 'a list)
    ...
    let getAt (l : 'a list) ~index:(index : int)  =
      (Base.List.nth l index : 'a option)
    ...
    let splitAt (l : 'a list) ~index:(index : int)  =
      (((take ~count:index l), (drop ~count:index l)) : ('a list * 'a list))
    let splitWhen (l : 'a list) ~f:(f : 'a -> bool)  =
      (match findIndex ~f:(fun _ -> fun element -> f element) l with
       | ((Some ((index, _)))) -> splitAt ~index l
       | None -> (l, []) : ('a list * 'a list))
    let updateAt (l : 'a list) ~index:(index : int)  ~f:(f : 'a -> 'a)  =
      (if index < 0
       then l
       else
    ...
    let removeAt (l : 'a list) ~index:(index : int)  =
      (if index < 0
       then l
       else
         (let (front, back) = splitAt ~index l in
    ...
    let insertAt (t : 'a list) ~index:(index : int)  ~value:(value : 'a)  =
      (let (front, back) = splitAt t ~index in append front (value :: back) : 
      'a list)
    let unzip = Base.List.unzip
    let unzip3 = Base.List.unzip3
    let sliding ?(step= 1)  (t : 'a t) ~size:(size : int)  =
      (let rec takeAllOrEmpty t n (current, count) =
         ...
         then reverse current
           ...
           (match t with
            | [] -> []
            | x::xs -> takeAllOrEmpty xs n ((x :: current), (count + 1))) in
       let rec loop t =
         if isEmpty t
         then []
         else
           (let sample = takeAllOrEmpty t size ([], 0) in
            if isEmpty sample
            then []
            else sample :: (loop (Base.List.drop t step))) in
       loop t : 'a t t)
    let intersperse (l : 'a list) ~sep  =
      (match l with
       ...
       | x::[] -> [x]
       | x::rest -> x ::
           (foldRight rest ~initial:[]
              ~f:(fun acc -> fun x -> sep :: x :: acc)) : 'a list)
...
module Option =
  struct
    ...
    let bind t ~f  =
      match t with | ((Some (x))) -> f x | None -> None
    ...
    let map2 (ta : 'a t) (tb : 'b t) ~f:(f : 'a -> 'b -> 'c)  =
      (match (ta, tb) with
       | (((Some (a))), ((Some (b)))) ->
           ((Some ((f a b))))
       | _ -> None : 'c t)
...
module Result =
  struct
    ...
    let mapError t ~f  =
      ...
      | ((Ok (value))) -> ((Ok (value)))
    let combine t =
      List.foldRight t ~initial:((Ok ([])))
        ~f:(map2 ~f:(fun b -> fun a -> a :: b))
    let toOption r =
      match r with
      | ((Ok (v))) -> ((Some (v)))
      | Error _ -> None
  ...
  struct
    ...
    let clamp n ~lower  ~upper  =
      ...
      then
        ...
        if
          (Base.Float.is_nan lower) ||
            ((Base.Float.is_nan upper) || (Base.Float.is_nan n))
        then Base.Float.nan
        else max lower (min upper n)
    let inRange n ~lower  ~upper  =
      if let open Base.Float in upper < lower
      then
        raise
          ((Invalid_argument
              (("~lower:" ^
    ...
    let squareRoot = sqrt
    let log n ~base  = let open Base.Float in (log10 n) / (log10 base)
    let isNaN = Base.Float.is_nan
    let isInfinite = Base.Float.is_inf
    let isFinite n = (not (isInfinite n)) && (not (isNaN n))
    let maximum x y =
      if (isNaN x) || (isNaN y) then nan else if y > x then y else x
    let minimum x y =
      if (isNaN x) || (isNaN y) then nan else if y < x then y else x
...
module Int =
  struct
    ...
    let (/) = (/)
    let (%) = Base.Int.(//)
    let power ~base  ~exponent  = let open Base.Int in base ** exponent
    let ( ** ) = Base.Int.( ** )
    let negate = (~-)
    let (~-) = (~-)
...
module Integer =
  struct
    ...
    let ofString string =
      match Z.of_string string with
      | value -> ((Some (value)))
      | exception _ -> None
    let zero = Z.zero
    let one = Z.one
    let isEven t = let open Z in (t mod (~$ 2)) = zero
    let isOdd t = let open Z in (t mod (~$ 2)) <> zero
    let add = Z.add
    let (+) = Z.(+)
    let subtract = Z.sub
    let (-) = subtract
    ...
    let negate = Z.neg
    let modulo (n : t) ~by:(by : t)  = (Z.rem n by : t)
    let remainder (n : t) ~by:(by : t)  = (Z.rem n by : t)
    let ( ** ) = Z.( ** )
    let power ?modulo  ~base:(base : t)  ~exponent:(exponent : int)  =
      match modulo with
      | ((Some (modulus))) ->
    ...
    let clamp n ~lower  ~upper  =
      ...
      then
        raise
          ((Invalid_argument ("~lower must be less than or equal to ~upper"))
          )
      else max lower (min upper n)
    let inRange n ~lower  ~upper  =
      if upper < lower
    ...
    let toInt t =
      ...
      then None
      else ((Some ((Z.to_int t))))
    let toInt64 t =
      if t > (ofInt64 Int64.max_int)
      then None
      else ((Some ((Z.to_int64 t))))
...
module String =
  struct
    ...
    let ofArray characters =
      let open Base in (Array.to_list characters) |> String.of_char_list
    ...
    let dropLeft ~count:(count : int)  (s : string) =
      (Base.String.drop_prefix s count : string)
    let dropRight ~count:(count : int)  (s : string) =
      (Base.String.drop_suffix s count : string)
    let split t ~on:(on : string)  =
    ...
    let includes t ~substring  =
      (Base.String.is_substring t ~substring : bool)
    let reverse = Base.String.rev
    let ofChar = Base.String.of_char
    let slice ?(to_= 0)  str ~from  = String.sub str from (to_ - from)
    let insertAt t ~index:(index : int)  ~value:(value : string)  =
      (let length = length t in
       ...
       String.concat "" [start; value; end_] : string)
...
module Map =
  struct
    ...
    let mapI t ~f  = Base.Map.mapi t ~f:(fun ~key -> fun ~data -> f key data)
    let filter = Base.Map.filter
    let partition m ~f  =
      Base.Map.partition_mapi m
        ~f:(fun ~key ->
              fun ~data ->
                if f ~key ~value:data then `Fst data else `Snd data)
    let find m ~f  =
      Base.Map.fold m ~init:None
        ~f:(fun ~key ->
              fun ~data ->
                fun matching ->
                  match matching with
    ...
    let fold m ~initial  ~f  =
      Base.Map.fold m ~init:initial
        ~f:(fun ~key -> fun ~data -> fun acc -> f acc ~key ~value:data)
    ...
    module Poly =
      struct
        ...
        let ofList l =
          Base.Map.Poly.of_alist_reduce l ~f:(fun _ -> fun curr -> curr)
    ...
    module Int =
      struct
        ...
        let ofList l =
          Base.Map.of_alist_reduce (module Base.Int) l
            ~f:(fun _ -> fun curr -> curr)
        let ofArray a = (Base.Array.to_list a) |> ofList
      end
    module String =
      struct
        ...
        let ofList l =
          Base.Map.of_alist_reduce (module Base.String) l
            ~f:(fun _ -> fun curr -> curr)
        let ofArray a = (Base.Array.to_list a) |> ofList
      end
  end
module Array =
  struct
    ...
    let sum (type a) t ((module M)  : (module Container.Sum with type t = a))
    ...
    let map2 (a : 'a array) (b : 'b array) ~f:(f : 'a -> 'b -> 'c)  =
      (let minLength = min (length a) (length b) in
       Base.Array.init minLength ~f:(fun i -> f (a.(i)) (b.(i))) : 'c array)
    let zip = map2 ~f:Tuple.make
    let map3 (arrayA : 'a array) (arrayB : 'b array) (arrayC : 'c array)
      ~f:(f : 'a -> 'b -> 'c -> 'd)  =
      let minLength =
        Base.min (length arrayA) (Base.min (length arrayC) (length arrayB)) in
      Base.Array.init minLength
        ~f:(fun i -> f (arrayA.(i)) (arrayB.(i)) (arrayC.(i)))
    let partition = Base.Array.partition_tf
    let splitAt a ~index  =
      ((Base.Array.init index ~f:(fun i -> a.(i))),
        (Base.Array.init ((length a) - 1) ~f:(fun i -> a.(index + i))))
    let splitWhen a ~f  =
      match findIndex a ~f:(fun _index -> fun element -> f element) with
      | None -> (a, [||])
      | ((Some ((index, _)))) -> splitAt a ~index
    let unzip = Base.Array.unzip
    let unzip3 t =
      ((map t ~f:Tuple3.first), (map t ~f:Tuple3.second),
    ...
    let intersperse array ~sep  =
      Base.Array.init (max 0 (((Array.length array) * 2) - 1))
    ...
    let slice ?to_  array ~from  =
      ...
      let sliceFrom =
        ...
        else max 0 (min (length array) ((length array) + from)) in
      let sliceTo =
        if defaultTo >= 0
        then min (length array) defaultTo
        else max 0 (min (length array) ((length array) + defaultTo)) in
      if sliceFrom >= sliceTo
      then [||]
      else
        Base.Array.init (sliceTo - sliceFrom)
          ~f:(fun i -> array.(i + sliceFrom))
    let sliding ?(step= 1)  a ~size  =
      let n = Array.length a in
      if size > n
      then [||]
      else
        initialize (1 + ((n - size) / step))
    ...
    let toIndexedList a =
      (Base.Array.fold_right a ~init:(((length a) - 1), [])
         ~f:(fun x -> fun (i, acc) -> ((i - 1), ((i, x) :: acc))))
        |> Base.snd
    let equal equal a b =
      ...
      then false
        ...
        then true
          ...
          (let rec loop index =
             if index = (length a)
             then true
             else (equal (a.(index)) (b.(index))) && (loop (index + 1)) in
           loop 0)
    let compare compare a b =
