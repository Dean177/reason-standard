...
let t_Array = () => {
  ...
  AT.check(
    ...
    List.indexedMap(~f=(i, _) => i, []),
    [],
  );
  AT.check(
    AT.list(AT.int),
    "indexedMap one element",
    List.indexedMap(~f=(i, _) => i, ['a']),
    [0],
  );
  AT.check(
    AT.list(AT.int),
    "indexedMap two elements",
    List.indexedMap(~f=(i, _) => i, ['a', 'b']),
    [0, 1],
  );
  AT.check(
    AT.array(AT.int),
  ...
  AT.check(
    AT.list(AT.int),
    "indexedMap empty list",
    List.indexedMap(~f=(_, n) => n + 1, []),
    [],
  );
  AT.check(
    AT.list(AT.int),
    "indexedMap one element",
    List.indexedMap(~f=(_, n) => n + 1, [(-1)]),
    [0],
  );
  AT.check(
    AT.list(AT.int),
    "indexedMap two elements",
    List.indexedMap(~f=(_, n) => n + 1, [(-1), 0]),
    [0, 1],
  );
  AT.check(
    AT.array(AT.int),
  ...
  AT.check(
    ...
    List.partition(~f=x => x mod 2 == 0, []),
    ([], []),
  );
  AT.check(
    AT.pair(AT.list(AT.int), AT.list(AT.int)),
    "partition one element",
    List.partition(~f=x => x mod 2 == 0, [1]),
    ([], [1]),
  );
  AT.check(
    AT.pair(AT.list(AT.int), AT.list(AT.int)),
    "partition four elements",
    List.partition(~f=x => x mod 2 == 0, [1, 2, 3, 4]),
    ([2, 4], [1, 3]),
  );
  AT.check(
...
let t_Int = () => {
  ...
  AT.check(AT.(option(int)), "ofString - --4", fromString("--4"), None);
  AT.check(
    AT.(option(int)),
...
let t_List = () => {
  ...
  AT.check(
    ...
    List.minimumBy(~f=x => x mod 12, [7, 9, 15, 10, 3, 22]),
    Some(15),
  );
  AT.check(
    AT.option(AT.int),
    "minimumBy empty list",
    List.minimumBy(~f=x => x mod 12, []),
    None,
  );
  ...
  AT.check(
    ...
    List.maximumBy(~f=x => x mod 12, [7, 9, 15, 10, 3, 22]),
    Some(10),
  );
  AT.check(
    AT.option(AT.int),
    "maximumBy empty list",
    List.maximumBy(~f=x => x mod 12, []),
    None,
  );
  ...
  AT.check(
    ...
    List.split_when(~f=x => x mod 2 == 0, [1, 3, 2, 4]),
    ([1, 3], [2, 4]),
  );
  AT.check(
    AT.pair(AT.list(AT.int), AT.list(AT.int)),
    "split_when at zero",
    List.split_when(~f=x => x mod 2 == 0, [2, 4, 6]),
    ([], [2, 4, 6]),
  );
  AT.check(
    AT.pair(AT.list(AT.int), AT.list(AT.int)),
    "split_when at end",
    List.split_when(~f=x => x mod 2 == 0, [1, 3, 5]),
    ([1, 3, 5], []),
  );
  AT.check(
    AT.pair(AT.list(AT.int), AT.list(AT.int)),
    "split_when empty list",
    List.split_when(~f=x => x mod 2 == 0, []),
    ([], []),
  );
  ...
  AT.check(
    AT.list(AT.int),
    "foldl empty list",
    List.foldLeft(~f=(x, acc) => [x, ...acc], ~initial=[], []),
    [],
  );
  AT.check(
    AT.list(AT.int),
    "foldl one element",
    List.foldLeft(~f=(x, acc) => [x, ...acc], ~initial=[], [1]),
    [1],
  );
  AT.check(
    AT.list(AT.int),
    "foldl three elements",
    List.foldLeft(~f=(x, acc) => [x, ...acc], ~initial=[], [1, 2, 3]),
    [3, 2, 1],
  );
  AT.check(
    AT.list(AT.int),
    "foldr empty list",
    List.foldRight(~f=(x, acc) => [x, ...acc], ~initial=[], []),
    [],
  );
  AT.check(
    AT.list(AT.int),
    "foldr one element",
    List.foldRight(~f=(x, acc) => [x, ...acc], ~initial=[], [1]),
    [1],
  );
  AT.check(
    AT.list(AT.int),
    "foldr three elements",
    List.foldRight(~f=(x, acc) => [x, ...acc], ~initial=[], [1, 2, 3]),
    [1, 2, 3],
  );
  AT.check(
    AT.int,
...
let t_Tuple = () => {
  ...
  AT.check(AT.int, "curry", Tuple2.curry(((a, b)) => a + b, 3, 4), 7);

  AT.check(AT.int, "uncurry", Tuple2.uncurry((a, b) => a + b, (3, 4)), 7);
...
let optionSuite = describe("Option", test => {
  describe("getUnsafe", test => {
    test("getUnsafe - Some(1", (expect) => {
      expect(getUnsafe(Some(1))) |> toEqual(AT.int, 1);
    });

    test("getUnsage - None", (expect) => {
      expect(() => Option.getUnsafe(None)) |> toRaise(Invalid_argument("option is None"))
    });
  });
};
...
let t_Result = () => {
  AT.check(
    ...
    Result.(fromOption(~error="error message", None: option(int))),
    Error("error message"),
  );
  AT.check(
    AT.result(AT.int, AT.string),
...
let () = {
  let (suite, exit) =
    Junit_alcotest.run_and_report("suite", [("tests", [
      ("Array", `Quick, t_Array),
      ("Char", `Quick, t_Char),
      ("Float", `Quick, t_Float),
      ("Fun", `Quick, t_Fun),
